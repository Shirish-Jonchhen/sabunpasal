                    ...
                    <li class="list-group-item">
                        <div class="d-flex align-items-center gap-3">
                            @if ($variant->product->images)
                                <img loading="lazy" src="{{ asset('storage/' . $variant->product->images->first()->image_path) }}"
                                    alt="Icon" width="40" height="40" class="rounded">
                            @else
                            ...
                            <div>
                                <strong>{!! preg_replace("/(" . preg_quote($search) . ")/i", "<span style='color:orange;'>$1</span>", $variant->product->product_name, 1) !!}
                                    / {{ $variant->attribute->attribute_value }}</strong><br>
                                <small>ID: {{ $variant->id }} | Unit: {{ $variant->attribute->attribute_value }} | Regular Price:
                                    {{ $variant->regular_price }} | Discounted Price: {{ $variant->discounted_price }} | Quantity:
                                    {{ $variant->stock_quantity }}</small>
                            </div>
